...
def generate_lipmix_submission(l,d, dat):
    ...
    for dis in np.arange(1,n_dist+1):
        ...
        for l in np.arange(1,total_l+1):
            ...
            for p in perm:
                tot_com = tot_com+1
                fn = dat.split(".dat")[0]+"_n_dist_"+str(dis)+"_"+str(list(p)).replace(", ","_")+"_"+str(tot_com)
                ...
                f.write("DIFFUSE\n")
                f.write("   0.9500    0.0000    1.0000    !! Volume fraction of the bilayer component\n")
                f.write("   2.232     2.232     2.232     !! Peak1 position (positive density)\n")
